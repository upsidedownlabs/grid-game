...
const Header: React.FC<HeaderProps> = ({
  ...
  const handleConfigClick = () => {
    if (onEnterConfigMode) {
      // Call the function to enter config mode via BLE
      onEnterConfigMode();
    }
      ...
      <div className="h-full max-w-[100vw] mx-auto flex justify-between items-center gap-1 sm:gap-3">
        ...
        <div className="flex items-center gap-1 sm:gap-3 min-w-0 shrink">
          ...
          <div className="hidden sm:flex items-center gap-1 md:gap-3">
            {/* Pen State Display */}
            <div className={`px-2 md:px-3 py-1 rounded-lg text-xs md:text-sm ${penState === 0
            ...
            {/* Mode Display */}
            <div className={`px-2 md:px-3 py-1 rounded-lg flex items-center gap-1 md:gap-2 text-xs md:text-sm ${currentMode === 0
            ...
            {isConfigMode && (
              <div className="px-2 md:px-3 py-1 rounded-lg text-xs md:text-sm bg-yellow-800/30 text-yellow-300 border border-yellow-600">
                <span className="flex items-center gap-1">
                  <span className="text-sm">⚙️</span>
                  <span className="hidden lg:inline">Config Mode</span>
                </span>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto scrollbar-none shrink-0">
          ...
          <div className="flex gap-1 sm:gap-2">
            {!connected ? (
              <button
                onClick={onConnect}
                className="flex items-center gap-1 sm:gap-2 cursor-pointer px-2 sm:px-3 py-2 bg-[#0077b6] text-white rounded-lg hover:bg-[#0096c7] transition-all text-xs sm:text-sm whitespace-nowrap"
              >
              ...
              <button
                ...
                className="flex items-center gap-1 sm:gap-2 cursor-pointer px-2 sm:px-3 py-1 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg hover:from-red-600 hover:to-red-700 transition-all shadow-md hover:shadow-lg text-xs sm:text-sm whitespace-nowrap"
              >
                <span className="text-sm">🔌</span>
          ...
          <div className="flex gap-1 sm:gap-2">
            <button
              onClick={onUndo}
              className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 cursor-pointer bg-gray-800 text-white rounded-lg hover:bg-gray-900 transition-all shadow-md hover:shadow-lg text-xs sm:text-sm whitespace-nowrap"
              title="Undo (Ctrl+Z)"
            >
              <span className="text-sm">↶</span>
            ...
            <button
              onClick={onRedo}
              className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 bg-gray-800 text-white cursor-pointer rounded-lg hover:bg-gray-900 transition-all shadow-md hover:shadow-lg text-xs sm:text-sm whitespace-nowrap"
              title="Redo (Ctrl+Y)"
            >
              <span className="text-sm">↷</span>
            ...
            <button
              onClick={showGame}
              className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 border border-blue-800 text-white cursor-pointer rounded-lg hover:bg-blue-600 transition-all shadow-md hover:shadow-lg text-xs sm:text-sm whitespace-nowrap"
            >
